  ...
  loadRecommendations(productId) {
    fetch(`${this.dataset.url}&product_id=${productId}&section_id=${this.dataset.sectionId}`)
      ...
      .then((text) => {
        ...
        var new_slider = new Swiper('[data-recommend-slider]', recommendSlider);
      })
      .catch((e) => {
        console.error(e);
      });
    ...
    summary.addEventListener('click', (e) => {
      ...
      accordions.forEach((acc) => {
        if (acc !== accordion) {
          ...
          acc.querySelector('[data-accordion-content]').style.maxHeight = 0;
        }
      });
...
let lastNavElement = document.querySelector('[data-menu-last]');
let lastnavCloseBtn = document.querySelector('[data-menu-last-button]');
let navContainer = document.querySelector('[data-drawer-nav-container]'); // Avoid multiple queries

if (lastNavElement && navContainer) {
  lastNavElement.addEventListener('click', function () {
    navContainer.classList.add('menu-drawer__navigation-container--show-sidebar');
  });
}

if (lastnavCloseBtn && navContainer) {
  lastnavCloseBtn.addEventListener('click', function () {
    setTimeout(function(){
      navContainer.classList.remove('menu-drawer__navigation-container--show-sidebar'); 
    }, 200)
  });
}
